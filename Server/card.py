...
class Card:
    ...
    def createFromJson(cls, j_dict):
        ...
        if card_type == CardType.NOBLE:
            return cls(card_type, j_dict["number"],
                       points=j_dict["points"], chips=j_dict["chips"])

        if card_type == CardType.NORMAL:
            return cls(card_type, j_dict["number"], j_dict["level"], j_dict["points"],
                       j_dict["gem_type"], j_dict["chips"])

    def __init__(self, card_type, number=0, level=0, points=0,
                 gem_type=Gemstone.NIL, chips=None):
